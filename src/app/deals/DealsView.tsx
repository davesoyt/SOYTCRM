...
function EditDealRow({ deal, stages, onDone }: { deal: Deal; stages: string[]; onDone: () => void }) {
  ...
  function save() {
    startTransition(async () => {
      await updateDeal(deal.id, { name, value: parseFloat(value) || 0, stage })
      onDone()
    })
  }
  ...
  return (
    <tr className="bg-blue-50">
      ...
      <td className="px-4 py-2 text-zinc-500">{deal.contactName ?? '—'}</td>
      <td className="px-4 py-2 text-zinc-500">{deal.companyName ?? '—'}</td>
      <td className="px-4 py-2">
...
export default function DealsView({ deals, pipelines }: { deals: Deal[]; pipelines: PipelineDTO[] }) {
  ...
  const pipelineDeals = useMemo(
    () => deals.filter(d => d.pipelineId === activePipeline?.id),
    [deals, activePipeline?.id],
  )

  const filtered = useMemo(() => {
    const q = query.toLowerCase().trim()
    if (!q) return pipelineDeals
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-1 mb-4 border-b border-zinc-200 overflow-x-auto">
        {pipelines.map(p => {
          ...
          return (
            ...
            >
              {p.name}
              <span className="rounded-full bg-zinc-100 px-1.5 py-0.5 text-xs text-zinc-600">{p.dealCount}</span>
              {p.isDefault && (
      ...
      {view === 'kanban' ? (
        activePipeline ? (
          <KanbanBoard
            deals={query ? filtered : pipelineDeals}
        ...
        <div className="bg-white rounded-xl border border-zinc-200 overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-zinc-100">
              {filtered.map(d =>
                editingId === d.id ? (
                  <EditDealRow key={d.id} deal={d} stages={stageLabels} onDone={() => setEditingId(null)} />
                ) : (
              ...
              {!filtered.length && (
                <tr><td colSpan={6} className="px-4 py-12 text-center text-zinc-400">
                  {query ? `No deals matching "${query}"` : 'No deals yet.'}
                </td></tr>
              )}
            </tbody>
          </table>
